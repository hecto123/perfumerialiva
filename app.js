...
const catalogoGrid=document.querySelector('.catalogo-grid') || document.getElementById('productsGrid') || document.querySelector('.products-grid');
...
function loadApp(){
	...
	fetch('perfumes.json')
		...
		.catch(error=>{
			console.error(error);
			if(perfumesData.length===0){
				if(catalogoGrid) catalogoGrid.innerHTML="<p class='empty'>No se pudo cargar el catálogo.</p>";
			} else {
				renderCatalogo();
				renderAdminProductList();
			}
...
function openIngredientesModal(index){
	...
	if(modalTitle) modalTitle.textContent=perfume.nombre;
	if(modalList) modalList.innerHTML=perfume.ingredientes.map(item=>`<li>${item}</li>`).join('');
...
function openAdminLogin(){
	if(adminLoginOverlay) adminLoginOverlay.classList.add('visible');
	if(adminLoginError) adminLoginError.textContent='';
	if(adminPasswordInput) adminPasswordInput.value='';
}

function closeAdminLogin(){
	if(adminLoginOverlay) adminLoginOverlay.classList.remove('visible');
}
...
function verifyAdmin(){
	...
	if(adminLoginError) adminLoginError.textContent='Clave incorrecta. Intenta nuevamente.';
}
...
function addProduct(){
	...
	if(!nombre||!precio||!ingredientes){
		if(productPreview) productPreview.textContent='Completa todos los campos.';
		return;
	}
	...
	const saveNewProduct=(imagenUrl)=>{
		...
		if(editIndex===null){
			...
			if(productPreview) productPreview.innerHTML=`<p>Producto agregado:</p><strong>${nombre}</strong>`;
		} else {
			perfumesData[editIndex]=productData;
			if(productPreview) productPreview.innerHTML=`<p>Producto actualizado:</p><strong>${nombre}</strong>`;
		}
	...
	if(imagenFile){
		...
		if(productPreview) productPreview.textContent='Debes seleccionar una imagen para el producto.';
	}
}

productImage && productImage.addEventListener('change', ()=>{
	...
	const reader=new FileReader();
	reader.onload=function(e){
		if(imagePreview) imagePreview.innerHTML=`<img src="${e.target.result}" alt="Vista previa" />`;
	};
...
function startEdit(index){
	...
	if(!perfume) return;
	editIndex=index;
	if(productName) productName.value=perfume.nombre;
	if(productPrice) productPrice.value=perfume.precio;
	if(productIngredients) productIngredients.value=perfume.ingredientes.join(', ');
	if(productPreview) productPreview.innerHTML=`<p>Editando: <strong>${perfume.nombre}</strong></p>`;
	if(cancelEditButton) cancelEditButton.style.display='inline-block';
	const addBtn=document.getElementById('addProductButton');
	if(addBtn) addBtn.textContent='Actualizar producto';
}
...
function resetForm(){
	...
	if(productIngredients) productIngredients.value='';
	if(productImage) productImage.value='';
	if(productPreview) productPreview.textContent='';
	if(imagePreview) imagePreview.innerHTML='';
	if(cancelEditButton) cancelEditButton.style.display='none';
	const addBtn=document.getElementById('addProductButton');
	if(addBtn) addBtn.textContent='Agregar producto';
}
